...
class DebugMem[T <: Data](size: Int, data: T) extends IndexedSeq[T] {
  private val version = """[0-9]+\.[0-9]+\.[0-9]+""".r findPrefixOf BuildInfo.version getOrElse("0.0.0")
  println(version)
  ...
  def apply(addr: Int): T = version match {
    case "3.6.0" => debugMem.get(addr)
    case _       => debugReg.get(addr)
  }

  def apply(addr: UInt): T = version match {
    case "3.6.0" => debugMem.get(addr)
    case _       => debugReg.get(addr)
  }

  def length: Int = version match {
